...
function getImageCdn($url, $width = 0, $height = 0, $fitIn = true, $webp = false, filters) {
    ...
    if ($url.includes('https://i.etsystatic.com')) {
        return getEtsyStatic($url, $width, $height);
    }

    if ($url.includes('redbubble.net')) {
        // return getRedBubbleStatic($url, $width, $height);
        return $url;
    }

    if ($url.includes('liveview')) {
        if (!$width) $width = $height;
        return $url.replace('/image/', '/image/' + $width + 'x' + $width + '/');
    }
...
function getEtsyStatic(url, w, h)
{
    let parts = url.split('/');
    let endPath = parts.pop();

    if (!w) w = 'N';
    if (!h) h = 'N';

    endPath = endPath.replace(/([\dN]+)x([\dN]+)/, w + 'x' + h);
    parts.push(endPath);

    return parts.join('/');
}

function getRedBubbleStatic(url, w, h)
{
    let parts = url.split('/');
    let endPath = parts.pop();
    ...
    if (w > 0 && h > 0) {
        endPath = endPath.replace(/(\d+)x(\d+)/, w + 'x' + h);
        /*endPath = endPath.replace(/(\d+)x/, w + 'x');
        endPath = endPath.replace(/x(\d+)/, 'x' + h);*/
    }
